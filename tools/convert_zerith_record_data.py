...
from lerobot.datasets.utils import write_tasks, write_episodes, write_stats
import pandas as pd
...
class HDF5ToLeRobotConverter:
    ...
    def _create_tasks_and_stats(self, output_path: Path) -> None:
        """Create tasks.parquet and stats.json files if they don't exist"""
        meta_path = output_path / "meta"
        
        # Create tasks.parquet if not exists
        if not (meta_path / "tasks.parquet").exists():
            print("Creating tasks.parquet...")
            tasks_df = pd.DataFrame({"task_index": [0]}, index=["Grab the red cube and place it on the plate"])
    ...
    def convert_single_file(self, hdf5_path: Path) -> None:
        ...
        try:
            ...
            for frame_idx in tqdm(range(n_frames), desc=f"Processing episode {self.episode_index} frames"):
                ...
                # Add task (timestamp, frame_index, index, episode_index are auto-generated by add_frame)
                frame["task"] = reader.task_name
